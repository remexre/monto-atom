...
export function activate() {
	apd.install();
	atom.commands.add("atom-workspace", "monto3:reconnect", reconnect);
	reconnect();
...
function reconnect() {
	...
	connect("http://localhost:28888").then(m => {
    	...
    	atom.notifications.addError(err.toString(), {
			detail: "You can reconnect in the Ctrl-Shift-P menu, as \"Monto3: Reconnect\"",
			dismissable: true
		});
	});
}
...
export function consumeIndie(registerIndie) {
	...
	subscriptions.add(atom.workspace.observeTextEditors((editor) => {
		const onChangeSubscription = editor.onDidStopChanging(ev => {
			// Don't fetch products if there was no net change.
			if(ev.changes.length === 0)
				return;
			// Otherwise, go for it.
			getProducts(editor, linter);
		});
		subscriptions.add(editor.onDidDestroy(() => onChangeSubscription.dispose()));
	}));
}

function bytesToRange(buffer, startByte, endByte) {
	atom.notifications.addWarning("TODO bytesToRange");
	return new Range(new Point(0, 0), new Point(0, 1));
}

function getProducts(editor, linter) {
		...
		monto.sendFile(path, editor.getText(), language).then(() => {
			...
			if(!language) {
				atom.notifications.addWarning("Could not determine Monto language for grammar " + grammar, {
					detail: "Language mappings are set in \"languages.json\" in this plugin. File an issue/PR.",
					dismissable: true
				});
				return;
			}
